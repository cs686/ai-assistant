...
export function PopupCard(props: IPopupCardProps) {
  ...
  useEffect(() => {
    ...
    const onMouseUp = () => {
      const selectedWord_ = editor.value
  ...
  useEffect(() => {
    const editor = editorRef.current;
    if (!editor) return;
    editor.dir = ["ar", "fa", "he", "ug", "ur"].includes(detectFrom)
      ? "rtl"
      : "ltr";
  }, [detectFrom, actionStr]);
  const [translatedLanguageDirection, setTranslatedLanguageDirection] =
    useState<Theme["direction"]>("ltr");
  useEffect(() => {
    setTranslatedLanguageDirection(
      ["ar", "fa", "he", "ug", "ur"].includes(detectTo) ? "rtl" : "ltr"
    );
  }, [detectTo]);
  ...
  useEffect(() => {
    ...
    const resizeHandle: ResizeObserverCallback = (entries) => {
      ...
      for (const entry of entries) {
        ...
        const yList = [maxY, $popupCard.offsetTop].filter(
          (item) => item > documentPadding
        );
        $popupCard.style.top = `${Math.min(...yList) || documentPadding}px`;
        const xList = [maxX, $popupCard.offsetLeft].filter(
          (item) => item > documentPadding
        );
        ...
        if ($translatedContent) {
          const translatedContentMaxHeight =
    ...
    return () => {
      queryPopupCardElement().then(
        ($popupCard) => $popupCard && observer.unobserve($popupCard)
      );
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const overflowCheck = (
      $popupCard: HTMLDivElement,
      movementXY: MovementXY
    ): number[] => {
      ...
      if (
        ...
        window.innerWidth -
          $popupCard.offsetLeft -
          movementX -
          $popupCard.offsetWidth >
          documentPadding
      ) {
          ...
          >
            {showSettings ? (
              ...
              <div style={props.containerStyle}>
                  ...
                  <div className={styles.popupCardHeaderButtonGroup}>
                    ...
                    >
                      <Button
                        size="mini"
                        kind={
                          translateMode === "analyze" ? "primary" : "secondary"
                        }
                    ...
                    >
                      <Button
                        size="mini"
                        kind={
                          translateMode === "mindmap" ? "primary" : "secondary"
                        }
                        onClick={() => {
                          setTranslateMode("mindmap");
                          // no need to change detectTo
                        }}
                      >
                        <RiMindMap />
                ...
                <div className={styles.popupCardContentContainer}>
                  ...
                  >
                    ...
                    <Dropzone onDrop={onDrop} noClick={true}>
                      {({ getRootProps, isDragActive }) => (
                        <div {...getRootProps()}>
                          {isDragActive ? (
                            <div className={styles.fileDragArea}>
                              {" "}
                              Drop file below{" "}
                            </div>
                          ) : (
                          ...
                          <div
                            style={{
                              ...
                              alignItems: "center",
                              paddingTop:
                                editableText && editableText !== originalText
                                  ? 4
                                  : 0,
                              height:
                                editableText && editableText !== originalText
                  ...
                  {/* <div><svg className="flex-1" ref={refSvg} /></div> */}
                  {originalText !== "" && (
                    <div
                      ...
                      {errorMessage ? (
                        ...
                        >
                          <div
                            ...
                            className={
                              styles.popupCardTranslatedContentContainer
                            }
                          >
                            <div>
            ...
            {props.showSettings && (
              <div className={styles.footer}>
                <StatefulTooltip
                  content={
                    showSettings ? t("Go to Translator") : t("Go to Settings")
                  }
                  showArrow
                  placement="right"
                >
